...
/**
 * 思路：
 * 1.首先将字符串进行trim()操作，去除前后的空格
 * 2.从头开始遍历，如果第一个字符不是数字，则直接返回0；
 * 3.如果是数字，则一直遍历到最后一个数字，或者遍历的长度超过10,
 * 就不再继续遍历，因为int的最大最小值长度只有10.
 * 4.将字符串转换成数组，如果超出长度就处理异常，将字符串减少一位，再转换。
 *
 */
public class Atoi {
    ...
    public int myAtoi(String str) {
        ...
        //确定结束位置
        for(int i= start;i<=start+10&&i<str.length();i++){
